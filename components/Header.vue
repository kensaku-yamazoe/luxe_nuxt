  ...
  <div>
    <header class="ly-header">
      <nav class="c-container flex items-center justify-between">
        <nuxt-link to="/" class="text-white">トップ</nuxt-link>
        <ul class="lg:flex hidden items-center justify-end gap-4">
          ...
          <li><nuxt-link to="/price" class="text-xl font-thin u-neon-text-pink01_narrow">Price</nuxt-link></li>
    ...
    <nav :class="{ 'is-open-menu': isOpen }" class="header-menu-mobile">
      <ul class="lg:hidden">
        ...
        <li><nuxt-link @click="closeMenu" to="/price" class="font-thin u-neon-text-pink01_narrow">Price</nuxt-link></li>
...
.c-hamburger {
  ...
  height: 21px;
  span {
    @apply absolute left-1/2 w-full h-1px bg-white transform -translate-x-1/2 transition duration-300 ease-in-out;
    &:nth-child(1) {
...
.header-menu-mobile {
  @apply fixed inset-0 z-40 flex flex-col items-center justify-center bg-tr_black50;
  opacity: 0;
  visibility: hidden;
